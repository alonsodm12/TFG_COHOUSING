...
import { getSolicitudesUsuario, aceptarSolicitud, rechazarSolicitud, eliminarSolicitud } from "../api/operations";

const SolicitudesPage: React.FC = () => {
    const { userId } = useParams();
    const [solicitudes, setSolicitudes] = useState<SolicitudesDTO[]>([]);
    const [loading, setLoading] = useState(false);
    const [refreshing, setRefreshing] = useState(false); // nuevo estado para botón refrescar
    ...
    const handleAccion = async (id: number, tipo: "aceptar" | "rechazar") => {
        ...
        try {
            ...
            // Elimina la solicitud localmente sin necesidad de refetch
            setSolicitudes(prev => prev.filter(s => s.id !== id));
        } catch (error) {
